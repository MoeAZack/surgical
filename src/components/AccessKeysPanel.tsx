 ...
 *  staff (e.g. other surgeons), each with its own label, without sharing the
 *  primary key. Only rendered for the primary session. */
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    try {
      const data = await apiJson("/api/keys", {
        ...
        body: JSON.stringify({ label: label.trim(), key: newKey.trim() })
      });
      setKeys(data.keys || []);
      setLabel("");
      setNewKey("");
  ...
  return (
    <div className="bg-white/5 backdrop-blur-md border border-white/10 rounded-2xl p-6 shadow-xl">
      ...
      <p className="text-xs text-white/50 leading-relaxed mb-4 flex items-start gap-1.5">
        ...
        {isRTL
          ? "أضف مفاتيح دخول إضافية لجراحين أو موظفين آخرين لتسجيل الدخول دون مشاركة مفتاحك الرئيسي."
          : "Issue additional sign-in keys for other surgeons or staff so they can log in without sharing your primary key."}
      </p>
      ...
      {!loading && (
        <div className="space-y-2 mb-4 max-h-[180px] overflow-y-auto pr-1">
          {keys.length === 0 ? (
            ...
            keys.map((k) => (
              <div key={k.id} className="flex items-center justify-between bg-white/5 border border-white/10 rounded-xl px-3 py-2">
                <div>
                  <span className="text-sm font-semibold text-white block">{k.label}</span>
